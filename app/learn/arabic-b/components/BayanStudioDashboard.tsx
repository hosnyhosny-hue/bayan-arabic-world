...
export default function BayanStudioDashboard() {
  ...
  async function loadCloud(): Promise<void> {
    const response = await fetch("/api/admin/bayan-platform/project", { cache: "no-store" });
    if (!response.ok) throw new Error(await response.text());
    const data = await response.json() as { project: BayanStudioProject };
    setProject(data.project);
    window.localStorage.setItem(BAYAN_STUDIO_STORAGE_KEY, JSON.stringify(data.project));
    window.alert("تم تحميل المسودة السحابية");
  }

  async function saveCloud(): Promise<void> {
    const response = await fetch("/api/admin/bayan-platform/project", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ project }) });
    if (!response.ok) throw new Error(await response.text());
    window.alert("تم حفظ المسودة في Firestore");
  }

  async function publishCloud(): Promise<void> {
    const response = await fetch("/api/admin/bayan-platform/publish", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ project }) });
    if (!response.ok) throw new Error(await response.text());
    const data = await response.json() as { releaseId: string };
    window.alert(`تم النشر للموقع: ${data.releaseId}`);
  }
      ...
      <section className={styles.workspace}>
        <header className={styles.topbar}>
          ...
          <div className={styles.topActions}>
            ...
            <button onClick={() => void loadCloud()} type="button">تحميل سحابي</button>
            <button onClick={() => void saveCloud()} type="button">حفظ Firestore</button>
            <button className={styles.primary} onClick={() => void publishCloud()} type="button">نشر للموقع</button>
