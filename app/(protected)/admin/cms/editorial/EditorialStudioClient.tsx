...
export default function EditorialStudioClient() {
  ...
  async function saveHeroDraft() {
    if (!hero.titleAr.trim()) {
      setMessage("أدخل عنوان الخبر الرئيسي أولًا.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تم حفظ الخبر الرئيسي كمسودة.");
    }
  }

  async function publishHeroNow() {
    if (!hero.titleAr.trim()) {
      setMessage("أدخل عنوان الخبر الرئيسي قبل النشر.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تم نشر الخبر الرئيسي الآن على BAYAN Pulse.");
    }
  }

  async function scheduleHero() {
    if (!hero.titleAr.trim()) {
      setMessage("أدخل عنوان الخبر الرئيسي قبل الجدولة.");
      return;
    }

    if (!hero.startsAt) {
      setMessage("حدد تاريخ ووقت بداية العرض للجدولة.");
      return;
    }

    const startsAt = apiDate(hero.startsAt);

    if (!startsAt || new Date(startsAt).getTime() <= Date.now()) {
      setMessage("يجب أن يكون موعد بداية العرض في المستقبل.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تمت جدولة الخبر الرئيسي بنجاح.");
    }
  }

  async function unpublishHero() {
    if (!hero.id) {
      setMessage("احفظ الخبر الرئيسي أولًا قبل إلغاء نشره.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تم إلغاء نشر الخبر الرئيسي.");
    }
  }

  async function saveTickerDraft() {
    if (!ticker.tickerTextAr.trim()) {
      setMessage("أدخل نص خبر الشريط أولًا.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تم حفظ خبر الشريط كمسودة.");
    }
  }

  async function publishTickerNow() {
    if (!ticker.tickerTextAr.trim()) {
      setMessage("أدخل نص خبر الشريط قبل النشر.");
      return;
    }
    ...
    if (saved) {
      ...
      setMessage("تم نشر خبر الشريط الآن.");
    }
  }

  async function scheduleTicker() {
    if (!ticker.tickerTextAr.trim()) {
      setMessage("أدخل نص خبر الشريط قبل الجدولة.");
      return;
    }

    if (!ticker.startsAt) {
      setMessage("حدد تاريخ ووقت بداية خبر الشريط.");
      return;
    }

    const startsAt = apiDate(ticker.startsAt);
    ...
    if (saved) {
      ...
      setMessage("تمت جدولة خبر الشريط.");
    }
  }

  async function publishExistingItem(item: EditorialItem) {
    const saved = await savePayload({
      ...item,
      type: item.type,
      status: "published",
    ...
    if (saved) {
      setMessage(
        item.type === "hero"
          ? "تم نشر الخبر الرئيسي الآن."
          : "تم نشر خبر الشريط الآن."
      );
    }
  }

  async function unpublishExistingItem(item: EditorialItem) {
    const saved = await savePayload({
      ...item,
      type: item.type,
      active: false,
    ...
    if (saved) {
      setMessage("تم إلغاء نشر المحتوى.");
    }
  }
                    ...
                    >
                      {status.key !== "live" ? (
                        <button
                          type="button"
                          onClick={() =>
                            void publishExistingItem(item)
                          }
                          className={
                            styles.quickPublishButton
                          }
                        >
                          نشر الآن
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            void unpublishExistingItem(item)
                          }
                          className={
                            styles.quickUnpublishButton
                          }
                        >
                          إلغاء النشر
                        </button>
