...
export default function ArabicBSceneMission({
  ...
  function speakText(text: string): void {
    if (!("speechSynthesis" in window)) {
      setVoiceMessage("تشغيل الصوت غير مدعوم في هذا المتصفح.");
      return;
    }
    window.speechSynthesis.cancel();
    ...
    utterance.onerror = () => {
      setIsSpeaking(false);
      setVoiceMessage("تعذر تشغيل الصوت. جرّب مرة أخرى.");
    };
        ...
        <article className={styles.sceneCard}>
          ...
          {scene.speakerAr && (
            <div className={styles.character}>
              <div>{world.character.avatar}</div>
          ...
          {scene.audioTextAr && (
            <div className={styles.audioPanel}>
              <div><small>النص الصوتي</small><strong>{scene.audioTextAr}</strong></div>
              <button type="button" onClick={() => speakText(scene.audioTextAr ?? "")}>{isSpeaking ? "يعمل الصوت…" : "🔊 استمع"}</button>
            </div>
          )}
